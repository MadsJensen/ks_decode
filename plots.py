...
for condition in conditions:
    for subject in subjects:
        if condition is 'all':
            scores = np.load('%s_time_gen_score_svm.npy' % subject)
        else:
            scores = np.load('%s_time_gen_score_%s_svm.npy' % (subject,
                                                             condition))
        ...
        plt.savefig('plots/%s_%s_diag.png' % (subject, condition))
        ...
        plt.savefig('plots/%s_%s_TG.png' % (subject, condition))
...
for condition in conditions:
    ...
    # Plot the diagonal (it's exactly the same as the time-by-time decoding above)
    fig, ax = plt.subplots()
    ax.plot(times, np.diag(scores_all), label='score')
    ax.axhline(.5, color='k', linestyle='--', label='chance')
    ax.set_xlabel('Times')
    ax.set_ylabel('AUC')
    ax.legend()
    ax.axvline(.0, color='k', linestyle='-')
    ax.set_title('Decoding MEG sensors over time')
    plt.savefig('plots/all_%s_diag_equal.png' % condition)
    ...
    ax.axhline(0, color='k')
    plt.colorbar(im, ax=ax)
    plt.savefig('plots/all_%s_TG_equal.png' % condition)
